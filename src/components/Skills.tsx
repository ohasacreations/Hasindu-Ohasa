        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {skillCategories.map((category, categoryIndex) => {
            ...
            return (
              <div 
                key={category.title}
                className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 hover:bg-white/15 hover:scale-105 hover:-translate-y-2 transition-all duration-500 group animate-fade-in-up"
                style={{ animationDelay: `${categoryIndex * 100}ms` }}
              >
                <div className="flex items-center space-x-3 mb-4">
                  <div className="bg-gradient-to-r from-purple-500/20 to-indigo-500/20 rounded-lg p-3 group-hover:scale-110 group-hover:rotate-12 transition-all duration-300">
                ...
                <div className="space-y-3">
                  {category.skills.map((skill, skillIndex) => (
                    <div 
                      key={skill}
                      className="bg-white/5 rounded-lg px-3 py-2 text-white/90 text-sm hover:bg-white/10 hover:scale-105 hover:text-purple-300 transition-all duration-300 cursor-default animate-slide-in-right"
                      style={{
